...
import {GET_TRIP, GET_TRIPS, ADD_TRIP, GET_ERRORS, GET_TRIPS_BY_ORGANIZER, TRIP_LOADING } from './types'
...
export const getTripsByOrganizer = id => dispatch => {
    dispatch(setTripLoading());
    axios
        .get(`/api/trips/organizer/${id}`)
        .then(res => dispatch({
            type: GET_TRIPS_BY_ORGANIZER,
            payload: res.data
        }))
        .catch(err => dispatch({
            type: GET_TRIPS_BY_ORGANIZER,
